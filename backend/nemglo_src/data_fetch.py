...
class Market:
    ...
    def get_emissions_intensity(self, emissions_type="total"):
        if emissions_type == "total":
            region_stats = get_total_emissions(self._start, self._end, self._cache, filter_regions=[self._region], by="interval")
            ## UPDATE to use by="halfhour"
            if self._intlength != 5:
                emissions = region_stats['Total_Emissions'].resample(str(self._intlength)+'min', 
                                    label='right', origin='end').sum()
                intensity = region_stats['Intensity_Index'].resample(str(self._intlength)+'min', 
                                    label='right', origin='end').mean()
                self._emissions = emissions[self._region]
                self._intensity = intensity[self._region]
            else:
                self._emissions = region_stats['Total_Emissions'][self._region]
                self._intensity = region_stats['Intensity_Index'][self._region]
            result = pd.DataFrame(self._intensity).reset_index()
            return result.rename(columns={self._region: 'Emissions_Intensity'})

        elif emissions_type == "marginal":
            region_stats = get_marginal_emissions(self._start, self._end, self._cache, redownload_xml=False)

            # Filter region
            reg_sum = region_stats[region_stats['RegionID'] == self._region]
            ...
            if self._intlength != 5:
                emissions = reg_sum['CO2E_EMISSIONS_FACTOR'].resample(str(self._intlength)+'min', 
                                    label='right', origin='end').max()
            else:
                emissions = reg_sum[['CO2E_EMISSIONS_FACTOR']]
            result = pd.DataFrame(emissions).reset_index()
            return result.rename(columns={'PeriodID': 'Time', 'CO2E_EMISSIONS_FACTOR': 'Emissions_Intensity'})
        else:
            raise Exception("Invalid `emissions_type` input. Must be either of ['total','marginal']")
